...
def rotate_file(host, filename, tmp_suffix, keep, logger):
    errors = []
    keep = int(keep)
    logger.log_message(logging.DEBUG, "Rotating archives")
    if isfile(host, filename + tmp_suffix):
        ...
        if isfile(host, filename + '.{}'.format(keep)):
            ...
            logger.log_message(
                logging.DEBUG, "Removing {}".format(filename_old))
        ...
        for number in range(keep, 0, -1):
            ...
            if isfile(host, filename_old):
                logger.log_message(
                    logging.DEBUG, "Renaming {} to {}".format(filename_old, filename_new))
        ...
        errors.append(
            "Temporary file '{}' doesn't exist, not rotating files".format(filename + tmp_suffix))
    return errors
